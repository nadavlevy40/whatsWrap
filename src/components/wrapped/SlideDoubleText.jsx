  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center px-6 gap-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-center">
        <p className="text-white/40 text-xs tracking-widest uppercase mb-2">Double Text Crown 👑</p>
        <h2 className="text-white text-3xl font-black leading-tight">Who can't wait<br />for a reply?</h2>
      </motion.div>
      ...
      <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.5, type: 'spring', damping: 12 }}
        ...
        <p className="text-white/40 text-sm mt-2">sent before getting a reply</p>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}
        ...
        <p className="text-white/60 text-sm mb-1">The Impatient One</p>
        <p className="text-white font-black text-4xl mb-1">{winner}</p>
        <p className="text-white/50 text-sm">{winnerCount.toLocaleString()} double texts</p>
      </motion.div>
