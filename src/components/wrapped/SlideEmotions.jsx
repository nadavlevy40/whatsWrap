...
export default function SlideEmotions({ data }) {
  const [p1, p2] = data.participants;
  const totalLaughs = data.laughCounts[p1] + data.laughCounts[p2];
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center px-6 gap-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-center">
        <p className="text-white/40 text-xs tracking-widest uppercase mb-2">Emotional Intelligence</p>
        <h2 className="text-white text-3xl font-black">Vibes & LOLs</h2>
      </motion.div>

      {/* Signature Emojis */}
      <div className="w-full space-y-3">
        <p className="text-white/40 text-xs text-center uppercase tracking-wider">Signature Emoji</p>
