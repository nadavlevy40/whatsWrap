...
3. Also ignore WhatsApp system messages like "end-to-end encrypted", "created group", "added", "left", etc.`;
    ...
    try {
      const insightsPrompts = {
        couple: `Analyze this WhatsApp conversation between a couple. Read the ENTIRE chat history to understand deep dynamics, inside jokes, and personality shifts. Return a JSON object with this exact structure:
{
  "couple": {
    "dynamic": "A creative 2-4 word relationship dynamic label (e.g. 'Black Cat & Golden Retriever')",
    "blackCatUser": "Name of the more aloof/mysterious person",
    "goldenRetrieverUser": "Name of the more enthusiastic/loyal person",
    "dynamicRoast": "A funny 2-sentence roast of their dynamic based on the whole chat",
    "evolution": "A 1-2 sentence summary of how the relationship evolved from start to present",
    "mostApologetic": "Name of the person who apologizes the most"
  }
}`,
        friends: `Analyze this WhatsApp group conversation between friends. Read the ENTIRE chat history to understand group dynamics, inside jokes, and personality shifts. Return a JSON object with this exact structure:
{
  "friends": {
    "unhingedQuote": { "sender": "Name", "text": "The most bizarre, out-of-context quote from the entire chat history" },
    "delusionalAward": { "user": "Name", "reason": "1-2 sentences on why this person is the most delusional based on their texts" },
    "therapist": "Name of the person who emotionally supports everyone else",
    "patient": "Name of the person who needs the most emotional support"
  }
}`,
        family: `Analyze this WhatsApp family group conversation. Read the ENTIRE chat history to understand each member's texting style and generational gaps. Return a JSON object with this exact structure:
{
  "family": {
    "boomerScores": { "Name1": 0-100, "Name2": 0-100 },
    "ignoredAward": { "user": "Name", "roast": "Funny 1-sentence comment about this person being left on read or ignored" }
  }
}
Note: boomerScores measures how 'boomer' each person's texting style is (100 = full boomer: uses full punctuation, sends 'ok.', forwards chain messages; 0 = very zoomer).`
      };

      const modePrompt = insightsPrompts[mode] || insightsPrompts.couple;
      const participantsStr = (parsed.participants || []).join(', ');
      const statsInfo = stats ? `\nProvided stats: ${JSON.stringify(stats)}` : '';

      const insightsCompletion = await openai.chat.completions.create({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: modePrompt },
          { role: 'user', content: `Participants: ${participantsStr}${statsInfo}\n\nFull chat history (condensed, chronological):\n${truncated}\n\nReturn only the JSON object.` },
        ],
        response_format: { type: 'json_object' },
        temperature: 0.7,
      });

      const insightsRaw = insightsCompletion.choices[0].message.content;
      parsed.aiInsights = JSON.parse(insightsRaw);
    } catch (_e) {
      // AI insights failed — slide will show fallback
    }
